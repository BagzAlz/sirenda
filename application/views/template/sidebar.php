...
<img  alt="" src="<?php echo base_url();?>file_upload/dp/<?php   if(!$dp->poto){ echo "no.jpg";}else{ echo $dp->poto;} ; ?>"/>
<div class="user-box">
<span class="name">
...
<?php }else{ ?>
	...
	<i class="fa fa-angle-right drop-icon"></i>
		...
		$jmlslash=count($slashLink);
		if($jmlslash>1){
		if($level1->link==$uri){ $link="style='backgrousnd:#33ccff;opacity:0.9;filter:alpha(opacity=10);'"; $aktif="active"; };
		}else{
		if($level1->link==$uri1){ $link="style='backgrosund:#33ccff;opacity:0.9;filter:alpha(opacity=10);'"; $aktif="active"; };
		}
		?>
