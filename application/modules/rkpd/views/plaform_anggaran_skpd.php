<div class="row">
  ...
  <div class="row">
    ...
    <ol class="breadcrumb">
      <li><a href="#">RKPD</a></li>
      <li class="active"><span>Plaform Anggaran SKPD</span></li>
    </ol>
    </div>
  </div>
  <br>
...
<h2 class="sadow05 black b">Plaform Anggaran SKPD <a href="javascript:add()" class="btn btn-primary pull-right">
<i class="fa fa-plus-circle fa-lg"></i> Tambah Plaform Anggaran SKPD
</a></h2>
</header>
...
<!--<link href="http://localhost/ajax_crud_datatables/assets/bootstrap/css/bootstrap.min.css" rel="stylesheet">-->
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('rkpd/open_anggaranskpd/'.$this->uri->segment(3).'')?>",
            "type": "POST"
        },
    ...
    function deleted(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data to database
          $.ajax({
            url:"<?php echo base_url();?>rkpd/delete_anggaranskpd/"+id,
            type: "POST",
            dataType: "JSON",
  ...
  function edit(id)
    ...
    //Ajax Load data from ajax
      $.ajax({
        url : "<?php echo site_url('rkpd/edit_anggaranskpd/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        { 
            $('[name="id_anggaranskpd"]').val(data.id_anggaranskpd); 
  ...
  <input type="hidden" value="" name="id_anggaranskpd"/> 
...
function add()
  ...
  $('.modal-title').html('<b>Tambah Plaform Anggaran SKPD</b>'); 
  }
...
function simpan()
{
  var id=$('[name="id_anggaranskpd"]').val();
  if(save_method=="add")
  {
      var link='<?php echo base_url("rkpd/add_anggaranskpd"); ?>'; 
  }
    else
  {
      var link='<?php echo base_url("rkpd/update_anggaranskpd"); ?>'; 
  }
