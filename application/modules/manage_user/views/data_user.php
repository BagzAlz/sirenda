...
<table id='table' class="tabel black table-striped table-bordered table-hover dataTable">
		  	<thead>			
				...
				<th class='thead' axis="string">Group User</th>			
				<th>&nbsp;</th>
			</thead>
</table>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('manage_user/ajax_open/'.$this->uri->segment(3).'')?>",
            "type": "POST"
        },
    ...
    function deleted(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data to database
          $.ajax({
            url:"<?php echo base_url();?>manage_user/deleted_UG/"+id,
            type: "POST",
            dataType: "JSON",
	...
	{
	$.ajax({
		url:"<?php echo base_url();?>manage_user/dropdownHak/"+id,
		type: "POST",
		data:"",
		 success: function(data)
				{
	...
	$("#msg").html("");
      ...
      $.ajax({
        url : "<?php echo base_url();?>manage_user/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {	 pilih(id)
...
$('#msg').html('<img src="<?php echo base_url();?>plug/img/load.gif"/> Loading...');
    ...
    var id=$('[name="Hak"]').val();
	if(save_method=="add"){
	var link='<?php echo base_url("manage_user/add_dataUser"); ?>'; }
	  else{
	 var link='<?php echo base_url("manage_user/update_profile"); ?>/'+id; }
