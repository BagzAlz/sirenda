<div class="row">
  ...
  <div class="row">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active"><span>Master Kegiatan</span></li>
    </ol>
    </div>
  </div>
  <br>
...
<h2 class="sadow05 black b">Master Kegiatan <a href="javascript:add()" class="btn btn-primary pull-right">
<i class="fa fa-plus-circle fa-lg"></i> Tambah Kegiatan
</a></h2>
</header>
...
<table id='table' class="tabel black table-striped table-bordered table-hover dataTable">
        ...
        <th class='thead' axis="string" width='250px'>Program</th>
        <th class='thead' axis="string" width='100px'>Kode Kegiatan</th>
        <th class='thead' axis="string" width='250px'>Kegiatan</th>
        <th width="40px">&nbsp;</th>
      </thead>
</table>
...
<!--<link href="http://localhost/ajax_crud_datatables/assets/bootstrap/css/bootstrap.min.css" rel="stylesheet">-->
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('data_master/open_kegiatan/'.$this->uri->segment(3).'')?>",
            "type": "POST"
        },
    ...
    function deleted(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data to database
          $.ajax({
            url:"<?php echo base_url();?>data_master/delete_kegiatan/"+id,
            type: "POST",
            dataType: "JSON",
  ...
  function edit(id)
    ...
    //Ajax Load data from ajax
      $.ajax({
        url : "<?php echo site_url('data_master/edit_kegiatan/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {   
...
<input type="hidden" value="" name="id_keg">
...
<div class="form-group">
  <label class="black col-lg-2 control-label">Bid Urusan </label>
   <div class="col-lg-9">
       <select id="bidang_urusan" name="kode_bid_urusan" class="form-control" >
                <option value="">--Pilih--</option>
...
<div class="form-group">
  <label class="black col-lg-2 control-label">Program </label>
   <div class="col-lg-9">
      <select id="program" name="kode_program" class="form-control">
        <option value="">--Pilih--</option>  
          <?php echo $row['nama_urusan']; ?>
            <?php foreach ($this->m_data_master->program() as $row): ?>
                  <option id="program" class="<?php echo $row['kode_bidang_urusan']; ?>" value="<?php echo $row['kode_program']; ?>">
                        <?php echo $row['nama_program']; ?>
                  </option><option id="program"  value="<?php echo $row['kode_program']; ?>">
                        <?php echo $row['nama_program']; ?>
                  </option>
            <?php endforeach; ?>
      </select>
...
<div class="form-group">
  <label class="black col-lg-2 control-label">Kode Kegiatan </label>
    <div class="col-md-9">
      <input type="text" class="form-control" value='' name="kode_kegiatan" placeholder="Kode Kegiatan">
    </div>
</div>
            
<div class="form-group">
  <label class="black col-lg-2 control-label">Kegiatan </label>
    <div class="col-md-9">
        <input type="text" class="form-control" value='' name="nama_kegiatan" placeholder="Nama Kegiatan">
    </div>
</div>
...
function add()
  ...
  $('.modal-title').html('<b>Tambah Kegiatan</b>'); 
  }
  
  
function simpan()
{
  var id=$('[name="id_keg"]').val();
  if(save_method=="add")
  {
      var link='<?php echo base_url("data_master/add_kegiatan"); ?>'; 
  }
    else
  {
      var link='<?php echo base_url("data_master/update_kegiatan"); ?>'; 
  }
